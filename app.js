...
function applyMove(token,record=true){
 ...
 if(record){history.push(token);undoStack.push(token);updateStatus();draw()}
}
...
function scramble(){const faces=['U','D','L','R','F','B'],mods=['',"'",'2'];let seq=[],last='';const len=+document.getElementById('scrambleLength').value;for(let i=0;i<len;i++){let f;do{f=faces[Math.floor(Math.random()*faces.length)]}while(f===last);last=f;seq.push(f+mods[Math.floor(Math.random()*mods.length)])}seq.forEach(m=>applyMove(m,true))}
...
function project(v,w,h){const [x,y,z]=camera(v),d=7,scale=Math.min(w,h)*0.18/(1-z/d);return[w/2+x*scale,h/2-y*scale,z]}
function stickerCorners(s){const gap=.08,half=.42;let u,v;if(Math.abs(s.n[0])){u=[0,1,0];v=[0,0,1]}else if(Math.abs(s.n[1])){u=[1,0,0];v=[0,0,1]}else{u=[1,0,0];v=[0,1,0]};const center=s.p.map((a,i)=>a+s.n[i]*.51);return [[-1,-1],[1,-1],[1,1],[-1,1]].map(([a,b])=>center.map((c,i)=>c+u[i]*a*(half-gap/2)+v[i]*b*(half-gap/2)))}
function draw(){
 ...
 const grad=ctx.createRadialGradient(w*.5,h*.45,30,w*.5,h*.5,Math.max(w,h)*.65);grad.addColorStop(0,'#26313c');grad.addColorStop(1,'#0b1015');ctx.fillStyle=grad;ctx.fillRect(0,0,w,h);
 const items=stickers.map(s=>{const pts=stickerCorners(s).map(p=>project(p,w,h));return{s,pts,z:pts.reduce((a,p)=>a+p[2],0)/4}}).sort((a,b)=>a.z-b.z);
 for(const it of items){ctx.beginPath();ctx.moveTo(it.pts[0][0],it.pts[0][1]);for(let i=1;i<4;i++)ctx.lineTo(it.pts[i][0],it.pts[i][1]);ctx.closePath();ctx.fillStyle=FACE_COLORS[it.s.c];ctx.fill();ctx.lineWidth=2;ctx.strokeStyle='#090c10';ctx.stroke()}
}
function renderAll(){updateStatus();draw();renderAlgorithmQueue()}

const moveGrid=document.getElementById('moveGrid');['U','U\'','U2','D','D\'','D2','L','L\'','L2','R','R\'','R2','F','F\'','F2','B','B\'','B2'].forEach(m=>{const b=document.createElement('button');b.className='btn move-btn'+(m.includes("'")?' inverse':'');b.textContent=m;b.onclick=()=>applyMove(m);moveGrid.appendChild(b)});
document.getElementById('resetBtn').onclick=resetCube;document.getElementById('scrambleBtn').onclick=scramble;document.getElementById('undoBtn').onclick=()=>{const t=undoStack.pop();if(t){applyMove(inverse(t),false);history.pop();renderAll()}};document.getElementById('clearHistoryBtn').onclick=()=>{history=[];undoStack=[];updateStatus()};
...
document.getElementById('loadAlgorithmBtn').onclick=loadAlgorithm;document.getElementById('nextAlgorithmBtn').onclick=()=>{if(algorithmIndex<algorithmMoves.length){applyMove(algorithmMoves[algorithmIndex]);algorithmIndex++;renderAlgorithmQueue()}};
